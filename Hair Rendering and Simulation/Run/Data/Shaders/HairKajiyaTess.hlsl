...
domain_out DomainMain(hull_tess_out input, float2 BarycentricCoords : SV_DomainLocation, OutputPatch<hull_out, 2> patch)
{
    ...
    // Rotates vector around scalp normal, using Rodrigues' formula
    float3 rotatedDir = RotateVectorAroundAxis(randDir, randRotation, patch[0].normal);
    
    float maxOffest = InterpolationRadius;
    // Creates offset in a rand radius, 0.1 at least so hair doesn't overlap
    offset = (maxOffest * (randNoise + 0.1f)) * rotatedDir; 
        
    /*
    Default is interpolating in a radius around a hair
    The other option is interpolation in a plane where
    the plane's normal is orthogonal to the hair's tangent
    */
    if (!InterpolateUsingRadius)
    {
        ...
        */
        ...
        offset = (maxOffest * (randNoise + 0.1f)) * sign(rawNoise) * jBasis;
    }
