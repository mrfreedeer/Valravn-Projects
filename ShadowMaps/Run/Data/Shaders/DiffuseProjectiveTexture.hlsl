...
{         
    ...
    float4 lightDepthPos = mul(Lights[0].ViewMatrix, modelTransform);
...
float4 PixelMain(v2p_t input) : SV_Target0
{
    ...
    float depthInShadowMap = depthTexture.Sample(diffuseSampler, input.lightDepthPosition.xy).r;
    float2 flippedUvs = float2(input.lightDepthPosition.x, 1.0f - input.lightDepthPosition.y);
    
    // For projecting textures. Much easier to debug
    if (input.lightDepthPosition.x >= 0.0f && input.lightDepthPosition.x <= 1.0f)
    {
        ...
        {
            if (depthInShadowMap > 0.0f && input.lightDepthPosition.z < 1.0f)
            {
                if (depthInShadowMap + DepthBias >= input.lightDepthPosition.z)
                {
                    return diffuseTexture.Sample(diffuseSampler, flippedUvs) /** input.color * ModelColor*/;
