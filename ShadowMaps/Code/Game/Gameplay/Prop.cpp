...
void Prop::InitiliazeLocalVertsCube()
{
	...
	AddVertsForQuad3D(m_vertsDiffuse, leftFrontBottom, rightFrontBottom, rightFrontTop, leftFrontTop, Rgba8::CYAN); // Front
	AddVertsForQuad3D(m_vertsDiffuse, rightFrontBottom, rightBackBottom, rightBackTop, rightFrontTop, Rgba8(255, 0, 255)); // Right
	AddVertsForQuad3D(m_vertsDiffuse, rightBackBottom, leftBackBottom, leftBackTop, rightBackTop, Rgba8::RED); // Back
	AddVertsForQuad3D(m_vertsDiffuse, leftBackBottom, leftFrontBottom, leftFrontTop, leftBackTop, Rgba8::GREEN); // Left
	AddVertsForQuad3D(m_vertsDiffuse, leftFrontTop, rightFrontTop, rightBackTop, leftBackTop, Rgba8::BLUE); // Top
	AddVertsForQuad3D(m_vertsDiffuse, leftBackBottom, rightBackBottom, rightFrontBottom, leftFrontBottom, Rgba8::YELLOW); // Bottom;
}
...
void Prop::InitiliazeLocalVertsGrid() {
	...
	for (int axisLineIndex = 1; axisLineIndex <= (int)halfMaxXUnit; axisLineIndex++) {
		...
		AddVertsForAABB3D(m_vertsDiffuse, xAxisLineAABB3, xAxisColor);
		AddVertsForAABB3D(m_vertsDiffuse, xNegAxisLineAABB3, xAxisColor);

		AddVertsForAABB3D(m_vertsDiffuse, yAxisLineAABB3, yAxisColor);
		AddVertsForAABB3D(m_vertsDiffuse, yNegAxisLineAABB3, yAxisColor);
	}
}

void Prop::InitializeLocalVertsSphere() {
	AddVertsForSphere(m_verts, m_sphereRadius, 8, 16);
	AddVertsForSphere(m_vertsDiffuse, m_sphereRadius, 8, 16);
...
void Prop::RenderMultiColoredCubeDiffuse() const
{
	g_theRenderer->BindTexture(g_textures[(int)GAME_TEXTURE::CompanionCube]);
	g_theRenderer->DrawVertexArray(m_vertsDiffuse);
}
...
void Prop::RenderGridDiffuse() const
{
	g_theRenderer->BindTexture(nullptr);
	g_theRenderer->DrawVertexArray(m_vertsDiffuse);
}
...
void Prop::RenderSphereDiffuse() const
{
	g_theRenderer->BindTexture(g_textures[(int)GAME_TEXTURE::TestUV]);
	//g_theRenderer->BindTexture(nullptr);
	g_theRenderer->DrawVertexArray(m_vertsDiffuse);
