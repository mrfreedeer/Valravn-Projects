	...
	m_faces(faces)
{
	...
	for (int vertIndex = 0; vertIndex < vertexes.size(); vertIndex++) {
		avgPos += vertexes[vertIndex];
	}

	m_middlePoint = avgPos / (float)vertexes.size();

	for (int faceIndex = 0; faceIndex < faces.size(); faceIndex++) { // Calculating the face middle point can helping with further calculations thus, it's precomputed here
...
void ConvexPoly3D::AddVertsForPoly(std::vector<Vertex_PCU>& verts, Rgba8 const& color) const
{
	for (int faceIndex = 0; faceIndex < m_faces.size(); faceIndex++) { // Calculating the face middle point can helping with further calculations thus, it's precomputed here
		...
		verts.emplace_back(face.m_middlePoint, color, Vec2::ZERO);
		verts.emplace_back(facePrevPoint, color, Vec2::ZERO);
		verts.emplace_back(m_vertexes[0], color, Vec2::ZERO);
...
std::vector<ConvexPoly3DEdge> Face::GetFaceEdges() const
{
	std::vector<ConvexPoly3DEdge> faceEdges;
	faceEdges.reserve(m_faceIndexes.size());

	if (m_owningPolygon) {
		Vec3 const& first = m_owningPolygon->m_vertexes[m_faceIndexes[0]];
		Vec3 prevVert = first;
		for (int faceVertIndex = 1; faceVertIndex < m_faceIndexes.size(); faceVertIndex++) {
			int vertInd = m_faceIndexes[faceVertIndex];
			Vec3 const& currentVert = m_owningPolygon->m_vertexes[vertInd];

			faceEdges.emplace_back(prevVert, currentVert);
			prevVert = currentVert;

		}


		faceEdges.emplace_back(prevVert, first);
