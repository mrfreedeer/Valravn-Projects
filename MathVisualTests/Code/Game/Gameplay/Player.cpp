...
void Player::Update(float deltaSeconds)
{
	...
	//m_orientation.m_yawDegrees += 1.0f;
...
Vec3 const Player::GetForward() const
{
	Mat44 orientationMat = m_orientation.GetMatrix_XFwd_YLeft_ZUp();
	return orientationMat.GetIBasis3D();
}

Vec3 const Player::GetDeltaDisplacement() const
{
	return m_position - m_prevPosition;
}

EulerAngles const Player::GetDeltaOrientation() const
{
	return m_mouseDeltaAngle;
}
...
void Player::UpdateInputFromKeyboard()
{
	...
	m_mouseDeltaAngle = EulerAngles(deltaMouseInput.x, -deltaMouseInput.y, 0.0f);
	m_orientation += m_mouseDeltaAngle;
}
